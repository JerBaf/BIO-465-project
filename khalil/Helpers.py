...
def simulate_regular_spiking_HH_neuron(input_current, simulation_time, init_dict={"vm":0,"m":0.05,"h":0.6,"n":0.32}):
    ...
    I_leak =  -1*gl*(vm-El) : amp
    I_Na =  -1*gNa*(m**3)*h*(vm-ENa) : amp
    I_K = -1*gK*n**4*(vm-EK) : amp
    ...
    # tracking parameters
    st_mon = b2.StateMonitor(neuron, ["vm", "I_e", "I_Na", "I_K","I_leak", "m", "n", "h", 
...
def pretty_print_spike_train_stats(voltage_monitor, spike_threshold):
    ...
    """
    nr_of_spikes, spike_times, ISI, mean_isi, spike_freq, var_isi = get_spike_stats(voltage_monitor, spike_threshold)
...
def get_spike_stats(voltage_monitor, spike_threshold):
    ...
    Args:
        ...
        spike_threshold (Quansaty): The spike threshold voltage. e.g. -50*b2.mV
...
def get_spike_time(voltage_monitor, spike_threshold):
    ...
    Returns:
        A list of spike times (Quansaty)
    """
    assert isinstance(voltage_monitor, b2.StateMonitor), "voltage_monitor is not of type StateMonitor"
    assert b2.units.fundamentalunits.have_same_dimHensions(spike_threshold, b2.volt),\
        "spike_threshold must be a voltage. e.g. brian2.mV"
 
    v_above_th = np.asarray(voltage_monitor.vm[0] > spike_threshold, dtype=int)
